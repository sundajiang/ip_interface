...
int main(int argc, char * argv[])
{
    ...
    */
	...
	#ifdef TEST
	strcpy( snd_buf.data,"hello , this is a test from ip_interface\n" );
	int k = msgsnd(nl_qid, &snd_buf, 2048, 0);
	if(k == 0)
        printf("ip_interface send already\n");
    else
        printf("ip_interface send error\n");

    printf("ip_interface snd_buff.mtype = %ld\n",snd_buf.mtype);
    printf("ip_interface snd_buff.mdest = %d\n",snd_buf.node);

    int size;
    mmsg_t rcv_buff;
    while (1)
	{
	    printf("========ip_interface waiting for data from nl_layer=======\n");
		size=msgrcv(vi_qid, &rcv_buff, sizeof(mmsg_t) - sizeof(long), 0, 0);
		printf("rcv data from nl:%s\n",rcv_buff.data);
        if (size < 0)
            break;
        printf("ip_interface %d bytes\n", size);
		printf("ip_interface rcv_buff->mtype = %ld\n" ,rcv_buff.mtype);
		printf("ip_interface rcv_buff->node_addr = %d\n" ,rcv_buff.node);
